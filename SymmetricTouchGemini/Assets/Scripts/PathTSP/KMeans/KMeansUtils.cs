...
public static class KMeans {
    ...
    public static KMeansResults Cluster(Vector3[] items, int clusterCount, int maxIterations, int seed) {
        ...
        for (int i = 0; i < items.Length; i++) {
            //Vector3 v = items[i];
            data[i] = new double[] { items[i].x, items[i].y, items[i].z };
        }
        return Cluster(data, clusterCount, maxIterations, seed);
    }

    public static KMeansResults Cluster(List<Vector3> items, int clusterCount, int maxIterations, int seed)
    {
    ...
    public static KMeansResults Cluster(double[][] data, int clusterCount, int maxIterations, int seed) {
        ...
        while (hasChanges && iteration < maxIterations) {
            ...
            totalDistance = CalculateClusteringInformation(ref data, ref clustering, ref means, ref centroidIdx, clusterCount, ref clusterItemCount);
    ...
    private static double CalculateClusteringInformation(ref double[][] data, ref int[] clustering, ref double[][] means, ref int[] centroidIdx, int clusterCount, ref int[] clusterItemCount) {
        ...
        for (int i = 0; i < data.Length; i++) {
            // Sum up the means
            row = data[i];
            clusterIdx = clustering[i]; // What cluster is data i assigned to
        ...
        for (int i = 0; i < data.Length; i++) {
            clusterIdx = clustering[i]; // What cluster is data i assigned to
            distance = CalculateDistance(data[i], means[clusterIdx]);
    ...
    private static bool AssignClustering(double[][] data, int[] clustering, int[] centroidIdx, int clusterCount) {
        ...
        for (int i = 0; i < data.Length; i++) {
            ...
            for (int k = 0; k < clusterCount; k++) {
                distance = CalculateDistance(data[i], data[centroidIdx[k]]);
